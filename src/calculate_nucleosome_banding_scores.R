...
get_banding_score = function(cell_subset) {
	...
	cell_subset.cleaned = cell_subset[, c("cell", "insert_size", "fragment_count")]
	missing_rows = do.call(rbind, lapply(size_range[! size_range %in% cell_subset.cleaned$insert_size], function(x) { data.frame(cell=cell_subset.cleaned[1, "cell"], insert_size=x, fragment_count=0)}))


	cell_subset.cleaned = rbind(cell_subset.cleaned, missing_rows) %>% arrange(insert_size)

	periodogram = spec.pgram(cell_subset.cleaned$fragment_count / max(cell_subset.cleaned$fragment_count), pad=0.3, tap=0.5, span=2, plot=F, fast=T)
